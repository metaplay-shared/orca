...
def main() -> None:
    parser = argparse.ArgumentParser(description="Initialize the Metaplay SDK for this project")
    parser.add_argument("--cli-path", default="metaplay", help="Path to the Metaplay CLI")
    parser.add_argument(
        "--sdk-version", default=None, help=f"Override the version pinned in {VERSION_FILE}"
    )
    ...
    if os.path.exists(SDK_DIR):
        ...
        result = subprocess.run(
            [args.cli_path, "init", "sdk", f"--sdk-version={version}", f"--sdk-directory={SDK_DIR}"],
            check=False,
        )
    ...
    for patch in patches:
        ...
        result = subprocess.run(["git", "apply", "--3way", patch], check=False)
        if result.returncode != 0:
            logger.error(
