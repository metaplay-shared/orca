...
namespace CloudCore.Tests.GameLogic {
	...
	public class PlayerMailTest {
		...
		public void MailWithLogHouseReward() {
			InitModels(new DateTime(2022, 6, 30, 12, 0, 0));
			PlayerRewardItem logHouseReward = new PlayerRewardItem(new LevelId<ChainTypeId>(ChainTypeId.FromString("LogHouse"), 3), amount: 2);
		...
		public void MailWithMultipleRewards() {
			InitModels(new DateTime(2022, 6, 30, 12, 0, 0));
			RewardCurrency goldReward = new RewardCurrency(CurrencyTypeId.Gold, 17);
			RewardCurrency gemReward = new RewardCurrency(CurrencyTypeId.Gems, 23);
			PlayerRewardItem itemReward = new PlayerRewardItem(new LevelId<ChainTypeId>(ChainTypeId.FromString("LogHouse"), 2), amount: 3);
