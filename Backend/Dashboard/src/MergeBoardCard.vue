...
<template lang="pug">
MCard(
  data-testid="player-merge-board-card"
  title="Merge Boards"
  ...
  div(class="tw-flex")
    ...
    div(class="tw-mb-4 tw-flex tw-w-1/4 tw-flex-col")
      div(
        ...
        span(:class="{ 'tw-text-neutral-400': island.state !== 'Open' }")
    ...
    div(class="tw-flex tw-flex-col tw-gap-1")
      ...
      div(
        ...
        div(
          ...
          :key="`${info.gridColStart}-${info.gridRowStart}`"
        ...
        table
          tbody
            ...
            tr
              ...
              td(v-if="currentIslandTasks")
                span(
                  v-for="(resource, index) in currentIslandTasks.items"
                  :key="index"
                  )
                  MTooltip(
            ...
            tr
              ...
              td(v-if="currentIslandTasks && currentIslandTasks.rewardItems.length != 0")
                span(
                  v-for="(reward, index) in currentIslandTasks.rewardItems"
                  :key="index"
                  )
...
// Subscribe to game configuration data, player state, and localization strings.
...
const { data: gameData } = useSubscription(() =>
  getGameDataByLibrarySubscriptionOptions(['Chains', 'Islands', 'IslandTasks'])
)
...
interface Task {
  ...
  islander: string
  id: string
  items: Array<{ type: string; level: number; count: number }>
  rewardItems: Array<{ type: string; count: number }>
}
/**
 * Transforms player's island data into a format suitable for the island selector UI.
 * Islands in this game represent different game areas/levels that players can unlock.
 * Each island has a state (Open, Hidden, Locked, etc.) that determines how it's displayed
 * and whether players can interact with it. This computed property prepares the data
 * for rendering island thumbnails with appropriate visual states.
 */
const islands = computed(() => {
  if (playerData.value) {
    const islands = playerData.value.model.islands
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument -- We know the keys of islands are strings, but TS can't infer that.
...
/**
 * Gets the player's progress data for the currently selected island.
 * This includes the island's merge board state, available tasks, and other player-specific data.
 * Reactive to selectedIsland changes, so the UI updates when users click different islands.
 */
...
/**
 * Finds the active task configuration for the selected island.
 * Islands can have multiple task slots, but only one active task at a time.
 * This computed property looks through the island's tasks, finds the active one,
 * and returns the corresponding task configuration from the game data.
 * Used to display task requirements and rewards to the user.
 */
const currentIslandTasks = computed<Task | undefined>(() => {
  if (selectedIslandData.value !== undefined && selectedIslandData.value?.tasks?.tasks !== undefined) {
    for (const key in selectedIslandData.value?.tasks.tasks) {
      if (key) {
        ...
        if (value !== undefined && gameData.value?.gameConfig.IslandTasks) {
...
/**
 * Processes island data into a format optimized for grid-based UI rendering.
 *
 * This complex computed property handles several important tasks:
 * 1. Combines static board layout (from game config) with dynamic item placement (from player state)
 * 2. Handles coordinate system conversion (game uses bottom-left origin, UI uses top-left)
 * 3. Manages multi-cell items (some items are 2x2 or larger and need special handling)
 * 4. Generates appropriate images and tooltips for each grid cell
 * 5. Applies visual effects like scaling to prevent grid gaps at certain zoom levels
 *
 * The result is an array of RenderInfo objects that the template can iterate over
 * to draw the merge board grid with proper positioning, images, and interactions.
 */
const selectedIslandRenderInfos = computed((): RenderInfo[] | undefined => {
  ...
  // eslint-disable-next-line complexity -- This function is necessarily complex due to the nature of the data transformations required for rendering the board.
...
/**
 * Determines which image to display for a given board tile.
 * Different tile types (Ground, ItemHolder, Ship) and items have different visual representations.
 * This function maps tile types and item data to their corresponding image file paths.
 */
...
/**
 * Finds what item (if any) is placed at a specific board coordinate.
 * Player's merge board items are stored as a flat array with x,y coordinates.
 * This helper function searches that array to find items at specific positions.
 */
function getItemForBoardLocation(x: number, y: number): any {
...
/**
 * Generates informative tooltip content for board tiles.
 * Provides different information based on whether the tile contains a special structure
 * (like docks or ships) or a mergeable item. Helps users understand what they're looking at.
 */
...
/**
 * Converts island IDs to player-friendly names using Metaplay's localization system.
 * Island IDs like 'Island_Beach' become readable names like 'Sunny Beach'.
 * Falls back to the raw ID if no localization exists.
 */
function getLocalizedIslandNameString(islandId: string): string {
  const islandTranslation = localizationData.value?.locs.en?.translations[`Island.${islandId}`]
  if (islandTranslation) {
    return islandTranslation
  } else {
    return islandId
  }
}
...
/**
 * Converts hero IDs to readable names using the localization system.
 * Essential for displaying hero names in task information.
 */
function getLocalizedHeroNameString(key: string): string {
  const heroTranslation = localizationData.value?.locs.en?.translations[`Hero.${key}`]
  if (heroTranslation) {
...
/**
 * Converts item/chain IDs to readable names using the localization system.
 * Used throughout the merge board interface to show meaningful item names.
 */
function getLocalizedItemNameString(key: string): string {
  const chainTranslation = localizationData.value?.locs.en?.translations[`Chain.${key}`]
  if (chainTranslation) {
    return chainTranslation
  } else {
    return key
  }
}
