...
export function GameSpecificPlugin(app: App): void {
  ...
  setGameSpecificInitialization(async (initializationApi) => {
    ...
    // Inject custom content into the player details page to render a player's heroes nicely.
    initializationApi.addUiComponent(
      'Players/Details/Tab0',
      {
        uniqueId: 'InventoryCard',
        vueComponent: async () => await import('./InventoryCard.vue'),
      },
      { position: 'before' }
    )
    ...
    initializationApi.addUiComponent(
      'Players/Details/Tab0',
      {
        uniqueId: 'HeroListCard',
        vueComponent: async () => await import('./HeroesCard.vue'),
      },
      { position: 'before' }
    )
    ...
    //   'Players/Details/Tab0',
    //   {
    //     uniqueId: 'IslandListCard',
    //     vueComponent: async () => await import('./IslandsCard.vue')
    //   },
    //   { position: 'before' }
    // )
