 ...
 */
export function GameSpecificPlugin(_: App): void {
    ...
    initializationApi.addInAppPurchaseContents([
      {
        ...
        getDisplayContent: (purchase): string[] => {
          const val: string[] = []
    ...
    initializationApi.addPlayerRewards([
      {
        ...
        getDisplayValue: (reward): string => `${reward.currencyId} x${reward.amount}`,
      },
      {
        $type: 'Game.Logic.PlayerRewardItem',
        getDisplayValue: (reward): string => `Level ${reward.chainId.level} ${reward.chainId.type} x${reward.amount}`,
      },
      {
        $type: 'Game.Logic.RewardResource',
        getDisplayValue: (reward): string => `${reward.currencyType} x${reward.amount}`,
      },
      {
        $type: 'Game.Logic.RewardItem',
        getDisplayValue: (reward): string => `${reward.itemType} lvl ${reward.level} x${reward.amount}`,
      },
    ])

    const gameData = await initializationApi.getGameDataByLibrary(['PlayerLevels'])
    initializationApi.addPlayerResources([
      {
        displayName: 'Level',
        getAmount: (playerModel): number => playerModel.level.level,
      },
      {
        displayName: 'Exp',
        getAmount: (playerModel): string => {
          return `${playerModel.level.currentXp}/${gameData.gameConfig.PlayerLevels[playerModel.level.level].xpToNextLevel}`
        },
      },
      {
        displayName: 'Gold',
        getAmount: (playerModel): number => playerModel.wallet.gold.purchased + playerModel.wallet.gold.earned,
      },
      {
        displayName: 'Gems',
        getAmount: (playerModel): number => playerModel.wallet.gems.purchased + playerModel.wallet.gems.earned,
      },
      {
        displayName: 'Energy',
        getAmount: (playerModel): number => playerModel.merge.energy.producedAtUpdate,
      },
      {
        displayName: 'Builders Free',
        getAmount: (playerModel): number => playerModel.builders.free,
      },
      {
        displayName: 'Builders Total',
        getAmount: (playerModel): number => playerModel.builders.total,
      },
      {
        displayName: 'Keys',
        getAmount: (playerModel): number =>
          playerModel.wallet.islandTokens.purchased + playerModel.wallet.islandTokens.earned,
      },
    ])
