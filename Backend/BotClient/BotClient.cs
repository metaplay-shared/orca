...
namespace Game.BotClient {
	...
	public class BotClient : BotClientBase {
		...
		protected override Task OnSessionStartedAsync(BotSessionStartedArgs args) {
			//((PlayerModel)args.PlayerModel).ClientListener = this;
		...
		void HandleSpecialBot() {
			...
			switch (_playerModel.PlayerId.Value) {
				...
				case 1:
					...
					PlayerContext.ExecuteAction(new PlayerForceChecksumMismatch(IslandTypeId.MainIsland));
					break;
				case 2:
		...
		void TickMainState() {
			...
			if (_playerModel.Wallet.Gems.Value < 50 &&
				...
				(_playerModel.InAppPurchaseHistorySummary.TotalNumPurchases == 0 ||
					_playerModel.InAppPurchaseHistorySummary.RecentPurchaseData.First().PurchaseTime.ToDateTime() <
			...
			foreach ((HeroTypeId heroTypeId, var heroModel) in _playerModel.Heroes.Heroes) {
				...
				if (_playerModel.Inventory.HasEnoughResources(heroModel.CurrentTask.Info) &&
					...
					PlayerContext.ExecuteAction(new PlayerFulfillHeroTask(heroTypeId));
				}
				...
				if (heroModel.CurrentTask.State == HeroTaskState.Fulfilled) {
					...
					if (_playerModel.Wallet.Currency(cost.Type).Value > cost.Amount) {
						PlayerContext.ExecuteAction(new PlayerSkipHeroTaskTimer(heroTypeId));
					}
				}
				...
				if (heroModel.CurrentTask?.State == HeroTaskState.Finished) {
					...
					if (heroBuilding != null) {
						PlayerContext.ExecuteAction(new PlayerClaimHeroTaskRewards(heroTypeId));
					}
				}
			}
			...
			foreach (var (key, value) in _playerModel.Logbook.Chapters) {
				foreach (var (logbookTaskId, logbookTaskModel) in value.Tasks) {
					if (logbookTaskModel.IsComplete && !logbookTaskModel.IsClaimed) {
						PlayerContext.ExecuteAction(new PlayerClaimLogbookTaskReward(logbookTaskId));
					}
				}

				if (value.State == ChapterState.Opening) {
					PlayerContext.ExecuteAction(new PlayerOpenLogbookChapter(key));
				}

				if (value.State == ChapterState.Complete) {
					PlayerContext.ExecuteAction(new PlayerClaimLogbookChapterReward(key));
				}
			}

			foreach (IslandModel island in _playerModel.Islands.Values) {
				if (island.State == IslandState.Revealing) {
					PlayerContext.ExecuteAction(new PlayerRevealIsland(island.Info.Type));
				}
				...
				if (island.State == IslandState.Locked &&
					...
					PlayerContext.ExecuteAction(new PlayerUnlockIsland(island.Info.Type));
				}
				...
				if (Random.Shared.Next(10) <= 1) {
					PlayerContext.ExecuteAction(new PlayerEnterIsland(island.Info.Type));

					for (var i = 0; i < _playerModel.Rewards.Count; i++) {
						PlayerContext.ExecuteAction(new PlayerClaimReward(island.Info.Type));
					}
				}
				...
				if (island.Tasks != null) {
					foreach (var (key, value) in island.Tasks.Tasks.ToMetaDictionary()) {
						if (board.HasItemsForTask(value.Info))
							PlayerContext.ExecuteAction(new PlayerFulfillIslandTask(island.Info.Type, key));
					}
				}
				...
				foreach (var kvp in board.LockArea.Areas) {
					if (kvp.Value == AreaState.Opening) {
						...
						if (areaInfo.UnlockProduct != null &&
							...
							_playerModel.Islands[island.Info.Type].MergeBoard.LockArea.AreaLockState(kvp.Key) == AreaState.Locked &&
							lastPurchase < MetaTime.Now + MetaDuration.FromMinutes(-5)) {
							lastPurchase = MetaTime.Now;
						...
						if (areaInfo.UnlockCost.Amount == 0 ||
							...
							if (areaInfo.UnlockProduct == null ||
								...
								_playerModel.Islands[island.Info.Type].MergeBoard.LockArea.AreaLockState(kvp.Key) == AreaState.Locked) {
								var result =
									PlayerContext.ExecuteAction(new PlayerOpenLockArea(island.Info.Type, kvp.Key));
					...
					for (int y = 0; y <= islandSize.BoardHeight; y++) {
						if (board[x, y] != null &&
							...
							if (item.Mine != null &&
								...
								if (SpawnsUsefulItem(item.Info, island, itemsFreeForMerge, requiredTaskItems, currentTaskItems))
									PlayerContext.ExecuteAction(new PlayerUseMine(island.Info.Type, x, y));
							}

							if (item.Mine != null && item.Mine.State == MineState.NeedsRepair) {
								PlayerContext.ExecuteAction(new PlayerRepairMine(island.Info.Type, x, y));
							}
							...
							if (item.Mine != null &&
								...
								if (SpawnsUsefulItem(item.Info, island, itemsFreeForMerge, requiredTaskItems, currentTaskItems))
									PlayerContext.ExecuteAction(new PlayerClaimMinedItems(island.Info.Type, x, y));
							}
							...
							if (item.HasItems && item.CanCreate && item.LockedState == ItemLockedState.Open) {
								...
								if (!hasEnergy) {
									...
									if (_playerModel.Wallet.Currency(costInfo.CurrencyType).Value >= costInfo.Cost) {
										PlayerContext.ExecuteAction(new PlayerFillEnergy(island.Info.Type));
										hasEnergy = true;
									}
								}
				...
				for (int x = 0; x <= islandSize.BoardWidth; x++) {
					for (int y = 0; y <= islandSize.BoardHeight; y++) {
						if (board[x, y] != null &&
							...
							soldAnything = true;
							PlayerContext.ExecuteAction(new PlayerSellMergeItem(island.Info.Type, x, y));
						}
					}
				}
		...
		private void OnGuildDiscoveryResponse(GuildDiscoveryResponse obj) {
			...
			{
				EnqueueOnActorContext(CreateGuildAsync);
			}
