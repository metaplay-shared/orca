...
namespace Game.Logic {
	...
	public class PlayerModel :
		...
		[MigrationFromVersion(fromVersion: 1)] // Replace 8 with the previous schema version in your project
		void MigrateLeagues()
		{
...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		protected override void GameInitializeNewPlayerModel(
			...
			#if SALES_DEMO || NETCOREAPP
			#if NETCOREAPP
			if (RuntimeOptionsBase.IsSalesEnvironment) {
				#endif

				Level.Reset(15, 100000, 100000);

				Wallet.Gems.Earn(10000);
				Wallet.Gold.Earn(10000);

				foreach (var (key, value) in GameConfig.Islands) {
					var islandModel = new IslandModel(value, GameConfig, MetaTime.Now, x => { });
					islandModel.ModifyState(IslandState.Open, GameConfig, MetaTime.Now, x => { }, ClientListener);
					foreach (var area in islandModel.MergeBoard.LockArea.Pattern) {
						islandModel.MergeBoard.LockArea.Areas[area] = AreaState.Open;
					}

					Islands.Add(key, islandModel);
				}

				foreach (var (key, value) in GameConfig.ResourceItems) {
					Inventory.UnlockResourceItem(value);
				}

				while (Heroes.CurrentHero != HeroTypeId.None) {
					Heroes.UnlockHero(GameConfig, model => { }, ClientListener, MetaTime.Now);
				}

				foreach (var (key, value) in Heroes.Heroes) {
					value.Level.Reset(10, 100000, 100000);
					value.AssignToBuilding(value.Info.DefaultBuilding);
				}

				UnlockFeature(FeatureTypeId.HomeIsland);
				UnlockFeature(FeatureTypeId.Map);
				UnlockFeature(FeatureTypeId.DiscountEvents);
				UnlockFeature(FeatureTypeId.ActivityEvents);
				UnlockFeature(FeatureTypeId.DailyTaskEvents);
				UnlockFeature(FeatureTypeId.SeasonalEvents);
				UnlockFeature(FeatureTypeId.Shop);
				UnlockFeature(FeatureTypeId.Logbook);
				UnlockFeature(FeatureTypeId.ItemRemoval);
				UnlockFeature(FeatureTypeId.HudButtonGold);
				UnlockFeature(FeatureTypeId.HudButtonGems);
				UnlockFeature(FeatureTypeId.HudButtonEnergy);
				UnlockFeature(FeatureTypeId.HudButtonBuilders);
				UnlockFeature(FeatureTypeId.HudButtonIslandTokens);

				foreach (var (key, value) in GameConfig.Triggers) {
					Triggers.Executed[key] = MetaTime.Epoch;
				}
		...
		protected override IEnumerable<MetaOfferGroupInfoBase> GetMetaOfferGroupsToActivate()
		{
