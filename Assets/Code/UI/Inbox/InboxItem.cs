		...
		public void Setup(PlayerMailItem playerMailItem) {
			...
			if (contents.Attachments?.Count > 0) {
				foreach (MetaPlayerRewardBase attachment in contents.Attachments) {
					if (attachment is RewardItem rewardItem) {
						string item = rewardItem.ItemType.Value;
						string itemAndLevel = item + rewardItem.Level;
						SpawnRewardIcon($"Chains/{item}/{itemAndLevel}.png", rewardItem.Amount);
					} else if (attachment is RewardResource rewardResource) {
						string item = rewardResource.CurrencyType.Value;
